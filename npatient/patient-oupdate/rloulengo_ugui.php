...
<td><input type="text" id="svnr" name="svnr" size="8" maxlength="10" value="<?php echo $svnr; ?>"></td>
</tr>
<tr><td>
Geschlecht: <br>
...
<?php
   // alle Krankheiten (= alle Datensätze) in Select-Option einbauen
   foreach ($krankheiten_liste as $krankheit) 
   { 
      $key=$krankheit['id_krankheit'];
      $value=$krankheit['bezeichnung'];
      
      // die in der Tabelle pa_besitzt vorhandenen Einträge als selected anzeigen
      $selected="";
      foreach ($krankheiten_der_person_liste as $person_krankheit) 
      { 
         // wenn die Person die aktuelle Krankheit eingetragen hat 
         if ($krankheit['id_krankheit']==$person_krankheit['id_krankheit']) 
            $selected="selected"; // diese Option als selected anzeigen
      } 
